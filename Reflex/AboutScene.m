...
-(id) init
{
	if( (self=[super initWithColor:ccc4(255, 255, 255, 255)])) {
        ...
        CCSprite *background=[CCSprite spriteWithFile:SHImageString(@"about")];
        background.position=ccp(SCREEN_WIDTH/2,SCREEN_HEIGHT/2);
        [self addChild:background];
        
        CCMenuItemImage *MainMenu=[CCMenuItemImage itemFromNormalImage:SHImageString(@"btn_back") selectedImage:SHImageString(@"btn_back_selected")  target:self selector:@selector(MainMenu)];
